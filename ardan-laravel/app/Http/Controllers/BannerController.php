...
class BannerController extends Controller
{
  ...
  public function read(Request $request) {
    ...
    $total = 0;
    $totalPage = 1;
    $cta = ($request->cta) ? $request->cta : null;
    $listData = Banners::select('banners.*')->orderBy($sortBy, $sortDir);
    if ($perPage != '~') {
        $listData->skip($offset)->take($perPage);
    }
    if ($search != null) {
        $listData->whereRaw('(banners.title LIKE "%'.$search.'%")');
    }
    if ($cta != null) {
        $listData->where('cta',$cta);
    }
    $listData = $listData->get();
    foreach($listData as $ld) {
      $ld->image_url = Storage::disk('public')->url('banner/'.$ld->image);
    }
    if ($search || $cta) {
        $total = Banners::orderBy($sortBy, $sortDir);
        if ($search) {
            $total->whereRaw('(banners.title LIKE "%'.$search.'%")');
        }
        if ($cta) {
            $total->where('cta', $cta);
        }
  ...
  public function get(Request $request) {
    if ($request->id) {
      ...
      $getData->image = Storage::disk('public')->url('banner/'.$getData->image);
      if ($getData) {
          $res = array(
                  'status' => true,
                  'data' => $getData,
