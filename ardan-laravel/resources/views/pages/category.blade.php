...
@section('screen')
<div class="layout-px-spacing" id="categoryPage">
                  ...
                  <table class="table table-hover table-striped table-bordered table-no-space">
                      ...
                      <tbody v-if="table.items.length > 0">
                        <tr v-for="(item,index) in table.items" :key="index">
                          <td>
                              <img alt="avatar" :src="item.image_url" class="img-thumbnail w-100 bg-dark rounded" v-if="item.image_url"/>
                              <span v-else>-</span>
                          </td>
                          <td>
              ...
              <div class="row">
                <div class="col-4">
                  <div class="form-group">
                    <label for="categoryImage" class="control-label">Image</label>
                    <label class="w-100 bg-dark">
                      <img :src="(form.data.image) ? form.data.image : 'https://placehold.co/200x200'" alt="" class="img-fluid w-100">
                      <input type="file" id="image" class="d-none" @change="previewImage($event)">
                    </label>
                    <input type="hidden" class="form-control" v-model="form.data.image" id="categoryImage">
                  </div>
                </div>
                <div class="col-8">
                  <div class="form-group">
                    <label for="categoryTitle" class="control-label">Title</label>
                    <input type="text" class="form-control" v-model="form.data.title" id="categoryTitle">
                  </div>
                  <div class="form-group mt-3">
                    <label for="categoryText" class="control-label">Text</label>
                    <input type="text" class="form-control" v-model="form.data.text" id="categoryText">
                  </div>
                  <div class="form-group mt-3">
                    <label for="categoryType" class="control-label">Type</label>
                    <select class="form-control" v-model="form.data.type" id="categoryType">
                      <option :value="opt" v-for="(opt,index) in opt.type" :key="index" v-text="opt"></option>
                    </select>
                  </div>
                </div>
              </div>
...
const vueDashboard = new Vue( {
  ...
  methods: {
      ...
      async doUpdate(id) {
        ...
        try {
          let req = await Api.categoryGet(payload)
          if(req.status == 200) {
      ...
      async doSave() {
        ...
        try {
          let req = false
          if(payload.id) {
            req = await Api.categoryUpdate(payload,this.users.access_token)
          } else {
            req = await Api.categoryCreate(payload,this.users.access_token)
          }
          if(req.status == 200) {
            let {data,status,msg} = req.data
            if(status){
      ...
      async doDelete(id) {
        ...
        try {
          let req = await Api.categoryDelete(payload,this.users.access_token)
          if(req.status == 200) {
            let {data,status,msg} = req.data
            if(status){
              this.notify('success','Success',msg)
