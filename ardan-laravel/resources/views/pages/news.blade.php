...
@section('screen')
<div class="layout-px-spacing" id="newsPage">
              ...
              <div class="row">
                <div class="col-12">
                  <div class="form-group">
                    <label for="newsImage" class="control-label">Image</label>
                    ...
                    <input type="hidden" class="form-control" v-model="form.data.image" id="newsImage">
                  </div>
                </div>
                <div class="col-12">
                  <div class="form-group">
                    <label for="newsTitle" class="control-label">Title</label>
                    <input type="text" class="form-control" v-model="form.data.title" id="newsTitle">
                  </div>
                  <div class="form-group mt-3">
                    <label for="newsText" class="control-label">Text</label>
                    <textarea class="form-control" v-model="form.data.text" id="newsText" rows="10"></textarea>
                  </div>
                  <div class="form-group mt-3">
  ...
  methods: {
      ...
      async doSave() {
        ...
        try {
          let req = false
          if(payload.id) {
            req = await Api.newsUpdate(payload,this.users.access_token)
          } else {
            req = await Api.newsCreate(payload,this.users.access_token)
          }
          if(req.status == 200) {
            let {data,status,msg} = req.data
            if(status){
      ...
      async doDelete(id) {
        ...
        try {
          let req = await Api.newsDelete(payload,this.users.access_token)
          if(req.status == 200) {
            let {data,status,msg} = req.data
            if(status){
              this.notify('success','Success',msg)
