...
@section('screen')
<div class="layout-px-spacing" id="eventsPage">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="row">
            ...
            <div class="col-12">
              <div class="form-group">
                <label for="eventsTitle" class="control-label">Title</label>
                <input type="text" class="form-control" v-model="form.data.title" id="eventsTitle">
              </div>
              <div class="form-group mt-3">
                <label for="eventsText" class="control-label">Text</label>
  ...
  methods: {
      ...
      async doSave() {
        ...
        try {
          let req = false
          if(payload.id) {
            req = await Api.eventsUpdate(payload,this.users.access_token)
          } else {
            req = await Api.eventsCreate(payload,this.users.access_token)
          }
          if(req.status == 200) {
            let {data,status,msg} = req.data
            if(status){
      ...
      async doDelete(id) {
        ...
        try {
          let req = await Api.eventsDelete(payload,this.users.access_token)
          if(req.status == 200) {
            let {data,status,msg} = req.data
            if(status){
              this.notify('success','Success',msg)
